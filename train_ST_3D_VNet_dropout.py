...
"""
MC-Dropout:
Average multi-runs of dropout outputs
...
res_dir = 'result/pancreas_final_VNet_dropout/'
split_name = 'pancreas'
...
def create_model(ema=False):
    net = nn.DataParallel(VNet(has_dropout=True))
    model = net.cuda()
    if ema:
...
def st_train(net, ema_net, optimizer, lab_loader, unlab_loader, test_loader):
    save_path = Path(res_dir) / 'con_{}_consistency_{}_st{}_VNet'.format(w_con[1].item(), consistency, st_save_step)
    save_path.mkdir(exist_ok=True)
    logging.info("Save path : ", save_path)
...
def pred_unlabel(net, pred_loader):
    ...
    net.module.dropout.train()
    net.module.branchs[0][1].train()
