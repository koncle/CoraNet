...
# 2, 1
os.environ['CUDA_VISIBLE_DEVICES'] = '4,5,6,7'  # '4,5,6,7'
# Parameters
num_class = 2
base_dim = 8
...
res_dir = 'result/pancreas_final_resnet18_dropout_0.5/'
split_name = 'pancreas'
cost_num = 3
...
def get_model_and_dataloader():
    ...
    data_root25 = '/data/DataSets/pancreas_pad25'
    data_root32 = '/data/DataSets/pancreas_pad32'

    trainset_lab = Pancreas(data_root25, split_name, split='train_lab')
    lab_loader = DataLoader(trainset_lab, batch_size=batch_size, shuffle=False, num_workers=0)

    trainset_unlab = Pancreas(data_root25, split_name, split='train_unlab', no_crop=True)
    unlab_loader = DataLoader(trainset_unlab, batch_size=1, shuffle=False, num_workers=0)

    testset = Pancreas(data_root25, split_name, split='test')
...
def st_train(net, ema_net, optimizer, lab_loader, unlab_loader, test_loader):
    save_path = Path(res_dir) / 'con_{}_consistency_{}_st20_all3'.format(w_con[1].item(), 1)
    save_path.mkdir(exist_ok=True)
    logging.info("Save path : ", save_path)

    writer = SummaryWriter(str(save_path), filename_suffix=time.strftime('_%Y-%m-%d_%H-%M-%S'))
    pretrained_path = Path(res_dir) / 'pretrain_con_{}_consistency_{}'.format(w_con[1].item(), 0.3)

    load_net_opt(net, optimizer, pretrained_path / 'best.pth')
    load_net_opt(ema_net, optimizer, pretrained_path / 'best_ema.pth')
    ...
    for epoch in tqdm(range(1, self_training_epochs)):
        ...
        for step, (data1, data2) in enumerate(zip(lab_loader, new_loader)):
            ...
            certain_loss = (loss_ce2 + dice_loss2) / 2 * consistency_weight
            ...
            const_loss1 = (consistency_dist1 * mask).sum() / (mask.sum() + 1e-16)
            consistency_dist2 = consistency_criterion(out2[1], out_ema[1])
            const_loss2 = (consistency_dist2 * mask).sum() / (mask.sum() + 1e-16)
            consistency_dist3 = consistency_criterion(out2[2], out_ema[2])
            const_loss3 = (consistency_dist3 * mask).sum() / (mask.sum() + 1e-16)
            uncertain_loss = consistency_weight * (const_loss1 + const_loss2 + const_loss3) / 3 * 0.5
...
def pred_unlabel(net, pred_loader):
    ...
    for (step, data) in enumerate(pred_loader):
        ...
        # N, C, H, W, D = img.shape
        # l = [((x // 32) + 1) * 32 for x in [H, W, D]]
        # pad = [(i-j)//2 for i, j in zip(l, (H, W, D))]
        # new_img = torch.zeros(N, C, l[0], l[1], l[2])
        # new_img[:, :, pad[0]:H+pad[0], pad[1]:W+pad[1], pad[2]:D+pad[2]] = img
        #
        # out = net(new_img)
        # plab0 = get_mask(out[0])[:, pad[0]:H+pad[0], pad[1]:W+pad[1], pad[2]:D+pad[2]]
        # plab1 = get_mask(out[1])[:, pad[0]:H+pad[0], pad[1]:W+pad[1], pad[2]:D+pad[2]]
        # plab2 = get_mask(out[2])[:, pad[0]:H+pad[0], pad[1]:W+pad[1], pad[2]:D+pad[2]]
        out = net(img)
...
if __name__ == '__main__':
    ...
    # logging.info(macs, params)
    # logging.info("VNet have {} paramerters in total".format(sum(x.numel() for x in model.parameters())))
